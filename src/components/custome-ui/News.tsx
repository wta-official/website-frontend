...
const News = async () => {
  try {
    ...
    const { results: blogs }: BlogsApiResponse = await fetchApi(`blogs/?page=1`);
    ...
    if (!blogs || blogs.length === 0) {
      return (
        <section className="p-6 sm:p-8 lg:p-16 text-white">
          ...
          <p className="text-lg">No news articles available at the moment. Please check back later.</p>
        </section>
      );
    }
      ...
      <section className="p-6 sm:p-8 lg:p-16 text-white">
        ...
        <HorizontalScroll>
          {blogs.map((article) => (
            ...
            >
              <Link href={`/blogs/${article.id}`} passHref className="group block">
                  <Image
                    src={article.image}
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
    ...
    return (
      <section className="p-6 sm:p-8 lg:p-16 text-white">
        <h2 className="text-2xl lg:text-4xl font-bold mb-6">In the news</h2>
        <p className="text-lg text-red-500">Failed to load news articles. Please try again later.</p>
      </section>
    );
  }
};
